...
def build_model_comparison(
    ...
    """
    Compare Ridge with block-only and unregularized OLS.
    """
    ...
    ols_metrics = load_summary_metrics(
        TEMPERATURE_OLS_SUMMARY_PATH,
        "temperature_ols",
    )

    ridge_metrics = ridge_summary_df[
        [
            "horizon",
            "loso_rmse",
            "loso_mae",
        ...
        columns={
            "loso_rmse": "temperature_ridge_rmse",
            "loso_mae": "temperature_ridge_mae",
            "loso_r_squared": (
                "temperature_ridge_r_squared"
            ),
        }
    )
    ...
    comparison_df[
        "ridge_vs_block_rmse_improvement_percent"
    ] = (
        (
            comparison_df["block_only_rmse"]
            - comparison_df[
                "temperature_ridge_rmse"
            ]
        )
        / comparison_df["block_only_rmse"]
        * 100
    ...
    comparison_df[
        "ridge_vs_block_mae_improvement_percent"
    ] = (
        (
            comparison_df["block_only_mae"]
            - comparison_df[
                "temperature_ridge_mae"
            ]
        )
        / comparison_df["block_only_mae"]
        * 100
    ...
    comparison_df[
        "ridge_vs_block_r_squared_improvement"
    ] = (
        comparison_df[
            "temperature_ridge_r_squared"
        ]
    ...
    comparison_df[
        "ridge_vs_ols_rmse_improvement_percent"
    ] = (
        (
            comparison_df["temperature_ols_rmse"]
            - comparison_df[
                "temperature_ridge_rmse"
            ]
        )
    ...
    comparison_df[
        "ridge_vs_ols_mae_improvement_percent"
    ] = (
        (
            comparison_df["temperature_ols_mae"]
            - comparison_df[
                "temperature_ridge_mae"
            ]
        )
...
def main() -> None:
    """Run nested-LOSO temperature Ridge models."""
    ...
    print("=" * 72)
    print("Temperature Month-Level Ridge")
    print("=" * 72)
    ...
    predictions_output.to_csv(
        OUTPUT_DIR
        / "temperature_month_level_ridge_predictions.csv",
        index=False,
    )

    coefficients_output.to_csv(
        OUTPUT_DIR
        / "temperature_month_level_ridge_coefficients.csv",
        index=False,
    )

    coefficient_summary_output.to_csv(
        OUTPUT_DIR
        / "temperature_month_level_ridge_coefficient_summary.csv",
        index=False,
    )

    diagnostics_output.to_csv(
        OUTPUT_DIR
        / "temperature_month_level_ridge_fold_diagnostics.csv",
        index=False,
    )

    inner_scores_output.to_csv(
        OUTPUT_DIR
        / "temperature_month_level_ridge_inner_cv_scores.csv",
        index=False,
    )
    ...
    print("=" * 72)
    print("Temperature Ridge Model Comparison")
    print("=" * 72)
    ...
    for row in comparison_output.itertuples(
        ...
        print(
            f"h={row.horizon}: "
            f"vs block RMSE="
            f"{row.ridge_vs_block_rmse_improvement_percent:+.2f}%, "
            f"vs OLS RMSE="
            f"{row.ridge_vs_ols_rmse_improvement_percent:+.2f}%, "
            f"ΔR² vs block="
            f"{row.ridge_vs_block_r_squared_improvement:+.4f}"
        )
