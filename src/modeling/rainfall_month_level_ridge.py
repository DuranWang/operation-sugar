"""
Rainfall Month-Level Ridge Baseline
...
def build_model_comparison(
    ...
    ridge_metrics = ridge_summary_df[
        ...
        columns={
            "loso_rmse": "rainfall_ridge_rmse",
            "loso_mae": "rainfall_ridge_mae",
            "loso_r_squared": (
                "rainfall_ridge_r_squared"
            ),
        }
    )
...
def main() -> None:
    """Run nested-LOSO rainfall Ridge models."""
    ...
    print("=" * 72)
    print("Rainfall Month-Level Ridge")
    print("=" * 72)
    ...
    predictions_output.to_csv(
        OUTPUT_DIR
        / "rainfall_month_level_ridge_predictions.csv",
        index=False,
    )

    coefficients_output.to_csv(
        OUTPUT_DIR
        / "rainfall_month_level_ridge_coefficients.csv",
        index=False,
    )

    coefficient_summary_output.to_csv(
        OUTPUT_DIR
        / "rainfall_month_level_ridge_coefficient_summary.csv",
        index=False,
    )

    diagnostics_output.to_csv(
        OUTPUT_DIR
        / "rainfall_month_level_ridge_fold_diagnostics.csv",
        index=False,
    )

    inner_scores_output.to_csv(
        OUTPUT_DIR
        / "rainfall_month_level_ridge_inner_cv_scores.csv",
        index=False,
    )
    ...
    comparison_output.to_csv(
        OUTPUT_DIR
        / "rainfall_ridge_model_comparison.csv",
        index=False,
    )

    print("=" * 72)
    print("Rainfall Ridge Model Comparison")
    print("=" * 72)
